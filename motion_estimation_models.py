#============================================================
#
#  Models for motion estimation (deformable registration)
#  Code extracted from: https://github.com/voxelmorph/voxelmorph/blob/master/pytorch/
...
if __name__ == "__main__":
    ...
    # ---------------- Random tensors ---------------------
    vref = torch.randn([8, 1, 32, 64, 64]).to(device)
    vt = torch.randn([8, 1, 32, 64, 64]).to(device)
    vol_size = [32, 64, 64]
    # ------------------------------------------------------
    model = Voxelmorph(vol_size, [16, 32, 32, 32], [32, 32, 32, 32, 32, 16, 16]).to(device)
    dvf = model(vref, vt)
